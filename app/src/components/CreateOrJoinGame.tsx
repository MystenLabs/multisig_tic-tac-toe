...
function createGameTxb({ myAddr, oppoAddr }: { myAddr: string, oppoAddr: string }) {
    ...
    txb.moveCall({
        ...
        arguments: [
            txb.pure.address(myAddr),
            txb.pure.address(oppoAddr),
        ]
    });
...
async function findGame(multiSigAddr: string) {
    ...
    const games = await suiClient.getOwnedObjects({
        owner: multiSigAddr,
        filter: { StructType: `${PACKAGE_ADDRESS}::multisig_tic_tac_toe::TicTacToe` },
...
function CreateOrJoinGame() {
    ...
    async function handleCreateGame() {
        ...
        const multiSigPublicKey = multisigPubKey(new Ed25519PublicKey(currentAccount!.publicKey), oppoPubKey);
        ...
        const game = resp.objectChanges?.filter((objChng) => {
            return objChng.type === 'created' && objChng.objectType === `${PACKAGE_ADDRESS}::multisig_tic_tac_toe::TicTacToe`;
        })[0] as {
            digest: string;
            objectId: string;
            objectType: string;
    ...
    async function handleJoinGame() {
        const opponentPubKeyArray = fromB64(opponentPubKey).slice(1);
        const oppoPubKey = new Ed25519PublicKey(opponentPubKeyArray);
        const multiSigPublicKeyAsO = multisigPubKey(oppoPubKey, new Ed25519PublicKey(currentAccount!.publicKey));
        const multiSigAddrAsO = multiSigPublicKeyAsO.toSuiAddress();
        const multiSigPubKeyAsX = multisigPubKey(new Ed25519PublicKey(currentAccount!.publicKey), oppoPubKey);
            ...
            <div className='tw-space-y-4 tw-w-full tw-space-x-4 tw-flex tw-flex-col tw-items-center'>
                ...
                <div className='tw-w-1/2 '>
                    ...
                    <div className='tw-space-x-4 tw-mt-4'>
                        <button className="btn btn-primary" onClick={handleCreateGame} disabled={!opponentValid}>Create Game (as X)</button>
                        <button className="btn btn-secondary" onClick={handleJoinGame} disabled={!opponentValid}>Join Game</button>
                    </div>
                </div>
            </div>
        </>
