...
use objects::{Mark, RowCol, Tictactoe};
use shared_crypto::intent::{Intent, IntentMessage};
use sui_keys::keystore::{Keystore, FileBasedKeystore, AccountKeystore};
use sui_sdk::apis::ReadApi;
use sui_sdk::json::SuiJsonValue;
use sui_sdk::rpc_types::{SuiObjectDataOptions, SuiData, SuiObjectDataFilter, SuiObjectResponseQuery, SuiParsedData, SuiMoveStruct, SuiTransactionBlockResponse, SuiTransactionBlockResponseOptions};
use sui_sdk::{SuiClient, SuiClientBuilder};
use sui_transaction_builder::DataReader;
use sui_types::Identifier;
use sui_types::base_types::{ObjectID, SuiAddress, ObjectRef};
use sui_types::crypto::{PublicKey, EncodeDecodeBase64, ToFromBytes};
use sui_types::gas_coin::GasCoin;
use sui_types::multisig::{MultiSigPublicKey, MultiSig};
use sui_types::programmable_transaction_builder::ProgrammableTransactionBuilder;
use sui_types::quorum_driver_types::ExecuteTransactionRequestType;
use sui_types::signature::GenericSignature;
use sui_types::transaction::{ProgrammableTransaction, TransactionData, Transaction};
...
pub struct SuiConfig {
    ...
    pub opponent: (SuiAddress, PublicKey),
...
impl SuiConfig {
    ...
    pub async fn fetch_my_marks(&self) -> Result<Vec<Mark>> {
    ...
    pub async fn fetch_game(&self, id: ObjectID) -> Result<Tictactoe> {
        let my_key = self.keystore.get_key(&self.my_address)?;
        let pub_keys = if self.playing_as_o {
            vec![self.opponent.1.clone(), my_key.public().into()]
        } else {
            vec![my_key.public().into(), self.opponent.1.clone()]
        };
    ...
    pub async fn create_game(&self) -> Result<SuiTransactionBlockResponse> {
        ...
        let pub_keys = if *playing_as_o {
            vec![opponent.1.clone(), my_key.public().into()]
        } else {
            vec![my_key.public().into(), opponent.1.clone()]
        };
        ...
        sui_client
            ...
            .single_move_call(
                ...
                vec![
                    ...
                    SuiJsonValue::from_str(&opponent.0.to_string())?,
                ],
            )
            .await?;
